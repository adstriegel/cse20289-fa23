...
# Check on lateness versus extra credit for the git side

echo "* Latest Commit Date - Files"

cd $DirRepo

# --format=%cd gives us the commit date
# --date=format d and m give us the day and month 

LatestDay=$(git log -1 --format=%cd --date=format:'%d' assignments/assignment01/*)
LatestMonth=$(git log -1 --format=%cd --date=format:'%m' assignments/assignment01/*)
LatestHour=$(git log -1 --format=%cd --date=format:'%H' assignments/assignment01/*)
LatestMinute=$(git log -1 --format=%cd --date=format:'%M' assignments/assignment01/*)

echo "  Last Commit Date (repo): $LatestMonth-$LatestDay $LatestHour:$LatestMinute"

ExtraCredit=0
LateSubmit=0

# Hack away to remove the leading zero
LatestMonth=$(echo "$LatestMonth" | bc)
LatestDay=$(echo "$LatestDay" | bc)
LatestHour=$(echo "$LatestHour" | bc)
LatestMinutes=$(echo "$LatestMinutes" | bc)

# Check if the latest month is less than or equal to 8
if [[ $LatestMonth -eq 9 ]]; then
    ExtraCredit=1
fi

if [[ $LatestMonth -eq 9 ]]; then
    if [[ $LatestDay -lt 2 ]]; then
        ExtraCredit=1
    elif [[ $LatestDay -eq 2 ]]; then 
        if [[ $LatestHour -lt 22 ]] ; then
            ExtraCredit=1
        elif [[ $LatestHour -eq 10 ]] ; then
            if [[ $LatestMinute -le 5 ]] ; then
                ExtraCredit=1
            fi
        fi
    fi
...
if [[ $LatestMonth -gt 9 ]]; then
    LateSubmit=1
fi

if [[ $LatestMonth -eq 9 ]]; then
    if [[ $LatestDay -ge 3 ]]; then
        LateSubmit=1
    elif [[ $LatestDay -eq 2 ]]; then
        if [[ $LatestHour -eq 22 ]]; then
            if [[ $LatestMinute -gt 5 ]]; then
                LateSubmit=1
            fi  
        elif [[ $LatestHour -gt 22 ]]; then
            LateSubmit=1
        fi
    fi
fi

if [[ ExtraCredit -eq 1 ]]; then
    echo "  Extra Credit on Git Side - Passes"
fi

if [[ LateSubmit -eq 1 ]]; then
    echo "  Last Commit was Late"
else 
    echo "  Last Commit was On Time for Full Credit"
fi
